...
%
%%-----Comment by Ashish----
%% Minor adaptation to make template usable for course project reports
%%-----End of Comment-----
...
%Add all authors name, Roll Number and Email address
% \author{First Author \\
...
\author{First Member 
  ...
  Group No: \\
  \{email1, email2, email3, email4\}@iitg.ac.in \\}
...
\maketitle
\begin{abstract}
Please use this format. You may go through the details mentioned in this document.
...
\section{Progress}
\begin{enumerate}
	\item In training/preprocessing part:
	\begin{enumerate}
		\item Data preprocessing involved combining answers and scores for questions, normalising the text and creating the corpus on which word embeddings had to be created.
		\item
		Created our own word embeddings by training a word2vec model using gensim on the given data.
		\item Trained a neural net to predict tags given a query.
	\end{enumerate}
	\item In the testing part:
	\begin{enumerate}
		\item Used cosine similarity to find the questions in dataset which resemble the most to this query.
	\end{enumerate}
\end{enumerate}
\section{Results}
Some of the results we obtained during various phases of development are:
\begin{enumerate}
	\item Word embeddings: A few examples for most similar words corresponding to the given word are(in decreasing of similarity):
	\begin{enumerate}
		\item django: flask, wagtail, mezzanine, drf, project
		\item api: apis, framework, webservice, endpoint, restful
		\item guniciorn: uwsgi, fastsgi, webl, nginx, apache2
		\item server: webserver, servers, application, app, client
	\end{enumerate}
	
	\item Neural net for tag prediction obtained a loss of 5.4205\% and a validation loss of 5.6137\%. Some of the examples for predicted tags for queries and ground truth tags are:
	\begin{enumerate}
		\item "mandatory try except get query referenced assignment error
		" -> "python" -> "django", "python", "python-3.x"
		...
	\end{enumerate}
	\item An example of our final prediction using cosine similarity:
	Given a query "Combine lists of lists" top 10 results we obtained are:
	\begin{enumerate}
		\item How to separate strings and tuples from list?
		\item Mutability of Lists
		\item Python: list of lists: replacing inner-lists with multiple/modified inner-lists
		\item ...
	\end{enumerate}
\end{enumerate}

\section{Conclusion}
Since the model training part is done our next task will be to work on the VS-code extension which will use the pretrained models to give appropriate results. Since the extension has to be written in javascript so we will have to look at tensorflow-js for further work.
